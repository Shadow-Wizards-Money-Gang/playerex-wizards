...
public final class ClientUtil {
	...
	/**
	 * Takes the input attribute and looks for the {@link ExAPI#PERCENTAGE_PROPERTY}
	 * property; if present, multiplies the input
	 * value by the property's multiplier. If not, then looks for the
	 * {@link ExAPI#MULTIPLIER_PROPERTY} property; if present,
	 * multiplies the input value by the property's multiplier. Else returns the
	 * input value.
	 * 
	 * @param attributeIn
	 * @param valueIn
	...
	public static double displayValue(final Supplier<EntityAttribute> attributeIn, double valueIn) {
		...
		if (attribute.hasProperty(ExAPI.PERCENTAGE_PROPERTY))
			return valueIn * Maths.parse(attribute.getProperty(ExAPI.PERCENTAGE_PROPERTY));
		if (attribute.hasProperty(ExAPI.MULTIPLIER_PROPERTY))
			return valueIn * Maths.parse(attribute.getProperty(ExAPI.MULTIPLIER_PROPERTY));
		return valueIn;
	}

	/**
	 * If the input value is positive, adds the "+" prefix. If the input attribute
	 * has the {@link ExAPI#PERCENTAGE_PROPERTY}
	 * property or the input FunctionBehaviour is
	 * {@link FunctionBehaviour#MULTIPLY}, appends the "%" suffix.
	 * 
	 * @param attributeIn
	 * @param behaviourIn
	 * @param valueIn
	 ...
	 * @since 3.5.0
	 */
	public static String formatValue(final Supplier<EntityAttribute> attributeIn, FunctionBehaviour behaviourIn,
			double valueIn) {
	...
	/**
	 * Looks at the input attribute's functions, and if there are any, formats them
	 * and adds them to the input list as tooltips.
	 * 
	 * @param tooltip
	 * @param attributeIn
	 */
	public static void appendChildrenToTooltip(List<Text> tooltip, final Supplier<EntityAttribute> attributeIn) {
		...
		for (var child : attribute.children().entrySet()) {
			...
			double value = function.value() * ((function.behaviour() == FunctionBehaviour.MULTIPLY
					&& !attribute2.hasProperty(ExAPI.MULTIPLIER_PROPERTY)
					&& !attribute2.hasProperty(ExAPI.PERCENTAGE_PROPERTY)) ? 100.0D : 1.0D);
	...
	/**
	 * Client-side. Sends a packet to the server to modify the attribute modifiers
	 * provided by PlayerEx by the input amount,
	 ...
	 */
	@SafeVarargs
	public static void modifyAttributes(final PacketType type,
			Consumer<BiConsumer<EntityAttributeSupplier, Double>>... consumers) {
		if (consumers != null) {
