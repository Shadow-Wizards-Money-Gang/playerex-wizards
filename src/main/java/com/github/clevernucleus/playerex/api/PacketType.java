...
/**
 * Holds four PacketTypes that tell the server what to do with the incoming
 * client-sent modify attributes packet.
 * 
...
public enum PacketType {
	/**
	 * Does nothing extra - just allows the server to modify PlayerEx's attribute
	 * modifiers.
	 */
	DEFAULT((byte) 0, (server, player, data) -> true),
	/**
	 * Only allows modification if the player's experience levels are greater than
	 * or equal to the required xp to level up.
	 * Also removes those experience levels from the player and adds 1 skill point.
	 */
	LEVEL((byte) 1, PacketType::level),
	/**
	 * Only allows modification if the player's skill points are greater than or
	 * equal to 1.
	 * Also subtracts one skill points from the player.
	 */
	SKILL((byte) 2, PacketType::skill),
	/**
	 * Only allows modification if the player's refund points are greater than or
	 * equal to 1.
	...
	private static boolean refund(final MinecraftServer server, final ServerPlayerEntity player,
