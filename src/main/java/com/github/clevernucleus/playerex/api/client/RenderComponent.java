...
public final class RenderComponent {
	...
	private RenderComponent(final Function<LivingEntity, Text> functionIn,
			final Function<LivingEntity, List<Text>> tooltipIn, final int dx, final int dy) {
	 ...
	 * @return
	 */
	public static RenderComponent of(final Function<LivingEntity, Text> functionIn,
			final Function<LivingEntity, List<Text>> tooltipIn, final int dx, final int dy) {
	...
	/**
	 * 
	 * @param attributeIn the text (and therefore tooltip) only display if the
	 *                    player has this attribute and it is not null (i.e.
	 ...
	 * @return
	 */
	public static RenderComponent of(final Supplier<EntityAttribute> attributeIn,
			final Function<Double, Text> functionIn, final Function<Double, List<Text>> tooltipIn, final int dx,
			final int dy) {
		return new RenderComponent(
				livingEntity -> DataAttributesAPI.ifPresent(livingEntity, attributeIn, Text.empty(), functionIn),
				livingEntity -> DataAttributesAPI.ifPresent(livingEntity, attributeIn, new ArrayList<Text>(),
						tooltipIn),
				dx, dy);
	}

	private boolean isMouseOver(float xIn, float yIn, float widthIn, float heightIn, int mouseX, int mouseY) {
		return mouseX >= (float) xIn && mouseY >= (float) yIn && mouseX < (float) (xIn + widthIn)
				&& mouseY < (float) (yIn + heightIn);
	}
