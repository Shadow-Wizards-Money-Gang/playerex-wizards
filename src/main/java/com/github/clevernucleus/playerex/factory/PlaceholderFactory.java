...
public final class PlaceholderFactory {
	...
	private static NameLevelPair nameLevelPair(final MinecraftServer server, final Collection<String> namesIn,
			final int indexIn) {
	...
	private static PlaceholderHandler top(Function<NameLevelPair, String> function) {
		return (ctx, arg) -> {
			...
			Collection<String> names = OfflinePlayerCache.getOfflinePlayerCache(server, Collections.emptySet(),
					opc -> opc.playerNames());
			int index = 1;
