...
public class ExScreen extends AbstractInventoryScreen<ExScreenFactory.Handler> {
	...
	private void addLayers(Page page, ExScreenFactory.Handler screenhandler, PlayerInventory playerInventory,
			Text text) {
	...
	protected void init() {
		super.init();
		this.clearChildren();
		this.addDrawableChild(new TabButtonWidget(this, PlayerExClient.INVENTORY, 0, 0, -28, true,
				NetworkFactoryClient::openInventoryScreen));
		...
		this.currentPage().forEachLayer(layer -> {
			...
			layer.children().stream().filter(e -> e instanceof ButtonWidget)
					.forEach(e -> this.addDrawableChild((ButtonWidget) e));
		});
	}
}