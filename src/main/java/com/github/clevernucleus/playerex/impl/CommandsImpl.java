...
public final class CommandsImpl {
	private static final Supplier<Collection<Identifier>> PRIMARIES = () -> Sets.newHashSet(ExAPI.CONSTITUTION.getId(),
			ExAPI.STRENGTH.getId(), ExAPI.DEXTERITY.getId(), ExAPI.INTELLIGENCE.getId(), ExAPI.LUCKINESS.getId());
	private static final SuggestionProvider<ServerCommandSource> SUGGESTION_PROVIDER = (context,
			builder) -> CommandSource.suggestIdentifiers(PRIMARIES.get(), builder);
	...
	private static void registerRefund(CommandNode<ServerCommandSource> root) {
		...
		ArgumentCommandNode<ServerCommandSource, EntitySelector> player = CommandManager
				.argument("player", EntityArgumentType.player()).executes(ctx -> {
					...
					if (refunded == 1) {
						ctx.getSource().sendFeedback(
								() -> Text.translatable("playerex.command.refund_alt", serverPlayerEntity.getName()),
								false);
					} else {
						ctx.getSource().sendFeedback(
								() -> Text.translatable("playerex.command.refund", refunded,
		...
		ArgumentCommandNode<ServerCommandSource, Integer> amount = CommandManager
				...
				.argument("player", EntityArgumentType.player()).executes(ctx -> {
					...
					return DataAttributesAPI.ifPresent(serverPlayerEntity, ExAPI.LEVEL, -1, value -> {
						...
						if (((IEntityAttribute) attribute).maxValue() - value < 1) {
							ctx.getSource()
									.sendFeedback(() -> (Text.translatable("playerex.command.attribute_max_error",
											Text.translatable(attribute.getTranslationKey()),
											serverPlayerEntity.getName())).formatted(Formatting.RED), false);
							return -1;
						}
						...
						ctx.getSource().sendFeedback(
								() -> Text.translatable("playerex.command.levelup_alt", serverPlayerEntity.getName()),
								false);
						return 1;
					});
				}).build();
		...
		ArgumentCommandNode<ServerCommandSource, Integer> amount = CommandManager
				.argument("amount", IntegerArgumentType.integer(1)).executes(ctx -> {
					...
					return DataAttributesAPI.ifPresent(serverPlayerEntity, ExAPI.LEVEL, -1, value -> {
						...
						if (adding == 1) {
							ctx.getSource().sendFeedback(
									() -> Text.translatable("playerex.command.levelup_alt",
											serverPlayerEntity.getName()),
									false);
						} else {
							ctx.getSource().sendFeedback(
									() -> Text.translatable("playerex.command.levelup", adding,
											serverPlayerEntity.getName()),
									false);
						}
	...
	private static void registerSkillAttribute(CommandNode<ServerCommandSource> root) {
		...
		ArgumentCommandNode<ServerCommandSource, Identifier> attribute = CommandManager
				.argument("attribute", IdentifierArgumentType.identifier()).suggests(SUGGESTION_PROVIDER)
				.executes(ctx -> {
					ServerPlayerEntity serverPlayerEntity = EntityArgumentType.getPlayer(ctx, "player");
					PlayerData playerData = ExAPI.PLAYER_DATA.get(serverPlayerEntity);
					...
					return DataAttributesAPI.ifPresent(serverPlayerEntity, primary, -1, value -> {
						...
						if (playerData.get(primary) < ((IEntityAttribute) attr).maxValue()) {
							if (PacketType.SKILL.test(ctx.getSource().getServer(), serverPlayerEntity, playerData)) {
								...
								ctx.getSource().sendFeedback(() -> Text.translatable("playerex.command.skill_attribute",
										Text.translatable(attr.getTranslationKey()), serverPlayerEntity.getName()),
										false);
								return 1;
							} else {
								ctx.getSource()
										.sendFeedback(
												() -> (Text.translatable("playerex.command.skill_attribute_error",
														serverPlayerEntity.getName())).formatted(Formatting.RED),
												false);
								return -1;
							}
						} else {
		...
		ArgumentCommandNode<ServerCommandSource, String> requiresSkillPoints = CommandManager
				.argument("requires", StringArgumentType.word())
				.suggests((ctx, builder) -> CommandSource.suggestMatching(Sets.newHashSet("true", "false"), builder))
				.executes(ctx -> {
					ServerPlayerEntity serverPlayerEntity = EntityArgumentType.getPlayer(ctx, "player");
					PlayerData playerData = ExAPI.PLAYER_DATA.get(serverPlayerEntity);
					Identifier identifier = IdentifierArgumentType.getIdentifier(ctx, "attribute");
						...
						if (playerData.get(primary) < ((IEntityAttribute) attr).maxValue()) {
							if (!requires.equals("true")) {
								...
								ctx.getSource().sendFeedback(() -> Text.translatable("playerex.command.skill_attribute",
										...
										false);
								return 1;
							} else if (PacketType.SKILL.test(ctx.getSource().getServer(), serverPlayerEntity,
									playerData)) {
								playerData.add(primary, 1);
								ctx.getSource().sendFeedback(() -> Text.translatable("playerex.command.skill_attribute",
										Text.translatable(attr.getTranslationKey()), serverPlayerEntity.getName()),
	...
	private static void registerRefundAttribute(CommandNode<ServerCommandSource> root) {
					...
					return DataAttributesAPI.ifPresent(serverPlayerEntity, primary, -1, value -> {
						...
						if (playerData.get(primary) > 0) {
							if (PacketType.REFUND.test(ctx.getSource().getServer(), serverPlayerEntity, playerData)) {
								playerData.add(primary, -1);
								ctx.getSource()
										.sendFeedback(() -> Text.translatable("playerex.command.refund_attribute",
								...
								ctx.getSource()
										.sendFeedback(
												() -> (Text.translatable("playerex.command.refund_attribute_error",
														serverPlayerEntity.getName())).formatted(Formatting.RED),
												false);
								return -1;
							}
							...
							ctx.getSource()
									.sendFeedback(
											() -> (Text.translatable("playerex.command.refund_attribute_unskilled",
													Text.translatable(attr.getTranslationKey()),
													serverPlayerEntity.getName()))
		...
		ArgumentCommandNode<ServerCommandSource, String> requiresRefundPoints = CommandManager
				.argument("requires", StringArgumentType.word())
				.suggests((ctx, builder) -> CommandSource.suggestMatching(Sets.newHashSet("true", "false"), builder))
				.executes(ctx -> {
					ServerPlayerEntity serverPlayerEntity = EntityArgumentType.getPlayer(ctx, "player");
							...
							if (!requires.equals("true")) {
								...
								ctx.getSource()
										.sendFeedback(() -> Text.translatable("playerex.command.refund_attribute",
												...
												false);
								return 1;
							} else if (PacketType.REFUND.test(ctx.getSource().getServer(), serverPlayerEntity,
									playerData)) {
								playerData.add(primary, -1);
								ctx.getSource()
										.sendFeedback(() -> Text.translatable("playerex.command.refund_attribute",
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess,
		...
		LiteralCommandNode<ServerCommandSource> root = CommandManager.literal("playerex")
