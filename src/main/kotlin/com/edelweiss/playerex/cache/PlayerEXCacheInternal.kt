...
class PlayerEXCacheInternal(
    ...
    companion object {
        ...
        /** In the case if a cache exists, it will return a function that provides the internal cache for use. */
        fun <T>ifCachePresent(server: MinecraftServer, fallback: T, function: (PlayerEXCacheInternal) -> T): T {
            val worldProperties = server.overworld.levelProperties
            val cache = (worldProperties as PlayerEXCacheData?)?.playerEXCache()
            return if (cache != null) function(cache) else fallback
        }
    }
    ...
    fun writeToNbt(): NbtList {
        ...
        for (uuid in this.cache.keys) {
            ...
            entry.putUuid(PlayerCacheKeys.UUID.key, uuid)
            entry.putString(PlayerCacheKeys.Name.key, uuidToPlayerNames.getOrDefault(uuid, ""))
            ...
            entry.put(PlayerCacheKeys.Keys.key, keys)
            list.add(entry)
        }
    ...
    fun readFromNbt(list: NbtList) {
        ...
        for (index in list.indices) {
            ...
            val keysCompound = entry.getCompound(PlayerCacheKeys.Keys.key)
            val uuid = entry.getUuid(PlayerCacheKeys.UUID.key)
            val name = entry.getString(PlayerCacheKeys.Name.key)
